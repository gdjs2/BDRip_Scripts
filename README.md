...
processes and plots video bitrate on the x-axis against B-frame QP on the y-axis.
Hover or click to pin a bitrate and compare each encoder's estimated QP.
A compact settings and queue sidebar leaves more space for the chart.
Reports retain individual clip measurements, endpoint means, figures, and logs.
Encoder settings and exact cropping are preserved.
New GUI tasks save their logs and results under `crf-tasks/` beside each input video.
