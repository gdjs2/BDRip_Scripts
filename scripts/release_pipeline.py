...
import bbcode_generator as bbcode
import nfo_generator as nfo
...
LOGGER = logging.getLogger("release_pipeline")
CONSOLE = Console(stderr=True)
HIDDEN_FILE_REGEX = r"(^|[\\/])\.[^\\/]+$"

COMMON_CONFIG_KEYS = {
    "movie_name", "chinese_name", "extra_description", "imdb_id", "douban_id",
    "encoder", "source", "movie_description_file", "description_api_base",
    "description_api_key_env", "screenshot_url_prefix_env", "thumbnail_suffix",
    "nfo_encoding", "imdb_overrides", "media_overrides", "poster_url", "plot",
    "release_date", "tmdb_api_base", "tmdb_api_token_env", "tmdb_api_key_env",
}
...
def setup_logging(root: Path) -> Path:
    ...
    old_handlers = {
        handler
        for logger in (LOGGER, bbcode.LOGGER)
...
def status_text(label: str, detail: str, current: int | None = None,
                total: int | None = None, step: int | None = None,
                total_steps: int = 7) -> Text:
...
def update_status(status: Any, label: str, detail: str,
                  current: int | None = None, total: int | None = None,
                  step: int | None = None, total_steps: int = 7) -> None:
        ...
        for aka in akas:
            ...
            if (
                ...
                and (
                    ...
                    or is_chinese_language(language)
                )
            ):
    ...
    for media_type, tmdb_id in media:
        try:
            ...
            LOGGER.warning(
                ...
                exc, media_type, tmdb_id,
            )
        else:
            for item in translations.get("translations", []) or []:
                if (
                    not isinstance(item, dict)
                    or not is_chinese_language(item.get("iso_639_1"))
                ):
                    continue
...
def discover_chinese_name(
    imdb_id: str, movie: Any, config: dict[str, Any]
) -> str:
...
def choose_movie_title(movie: Any, requested: str = "", input_fn=input) -> str:
    ...
    while True:
        answer = input_fn(f"Select movie name [1-{len(candidates)}] (default 1): ").strip()
        if not answer:
            return candidates[0]
...
def audio_release_token(media_file: Path) -> str:
    ...
    def track_name(track: Any) -> str:
        return " ".join(str(getattr(track, key, "") or "") for key in
                        ("commercial_name", "format", "codec_id", "title")).casefold()

    def is_core(track: Any) -> bool:
        name = track_name(track)
        if any(codec in name for codec in ("ac-3", "e-ac-3", "aac", "pcm")):
            return True
        return "dts" in name and not any(codec in name for codec in ("dts-hd", "dts:x", "dts-x", "xll"))
    ...
    def lossless_score(track: Any) -> tuple[int, int]:
        name = " ".join(str(getattr(track, key, "") or "") for key in
                        ("commercial_name", "format", "title")).casefold()
        priority = 5 if "atmos" in name else 4 if "truehd" in name else 3 if "dts-hd" in name else 1
    ...
    if "atmos" in name and "truehd" in name:
        ...
        codec = re.sub(r"[^A-Za-z0-9-]+", ".", str(track.format or track.codec_id)).strip(".")
...
def source_mkv(root: Path) -> Path:
    ...
    if len(files) != 1:
        raise ValueError(f"Expected exactly one MKV directly in {streams}; found {len(files)}")
    return files[0]
...
def make_release_name(movie: Any, variant: str, audio_token: str,
                      selected_title: str = "") -> str:
    spec = VARIANTS[variant]
    parts = [
        release_title(selected_title or movie.title), str(movie.year), "1080p", "BluRay",
        f"{spec['codec']}{spec['bit_depth']}", audio_token,
    ]
...
def split_legacy_config(config: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
...
def absolutize_config_paths(config: dict[str, Any], base: Path, keys: Iterable[str]) -> None:
...
def read_or_migrate_pipeline_config(root: Path) -> tuple[Path, dict[str, Any], bool]:
    ...
    for variant, spec in VARIANTS.items():
        configs = sorted(
            path for path in (root / spec["artifact"]).glob(f"*{CONFIG_SUFFIX}")
        ...
        absolutize_config_paths(
            variant_config, configs[0].parent,
            ("file_path", "screenshots_dir", "encoder_info_file", "output", "torrent_output"),
        )
...
def prepare(root: Path, imdb_id: str, douban_id: str, selected: list[str],
            force: bool, requested_title: str = "") -> None:
    root = root.resolve()
    if not root.is_dir():
    ...
    bbcode.load_env(Path.cwd() / ".env")
    bbcode.load_env(root / ".env")
    ...
    with CONSOLE.status(
        ...
        for index, variant in enumerate(selected, 1):
            update_status(
                status, "Preparing releases", "creating directories",
                index, len(selected), total_steps=len(selected),
            )
            ...
            release_name = make_release_name(movie, variant, audio_token, selected_title)
...
def load_variant_config(root: Path, variant: str) -> tuple[
    ...
    if path.is_file():
        ...
        if not isinstance(variant_config, dict):
            raise ValueError(f"No {variant} object in {path}; run prepare --variant {variant}")
...
def last_log_for(root: Path, variant: str, configured: str) -> Path:
    if configured:
        ...
        return path
    candidates = [
        path for path in (root / "Logs").glob(f"*{variant}*_encode_*.txt")
        if is_visible_file(path)
    ]
    if not candidates:
        candidates = [
            path for path in (root / "Logs").glob(f"*{variant}*.txt")
            if is_visible_file(path)
        ]
    if not candidates:
...
def extract_encoder_info(log_path: Path, variant: str) -> str:
    ...
    if variant == "x265":
        patterns = [
            ...
            r"x265 \[info\]: frame I:", r"x265 \[info\]: frame P:",
            r"x265 \[info\]: frame B:", r"x265 \[info\]: Weighted P-Frames:",
            r"x265 \[info\]: Weighted B-Frames:", r"x265 \[info\]: consecutive B-frames:",
        ]
    else:
        patterns = [
    ...
    if len(result) < required:
        raise RuntimeError(f"Only found {len(result)} encoder summary lines in {log_path}")
    return "\n".join(result)
...
def ptgen_description(douban_id: str, base: str, api_key: str = "") -> str:
    ...
    if not description:
        raise RuntimeError("PT-Gen response did not contain a formatted movie description")
    return description
...
def imdb_description(imdb_id: str, douban_id: str) -> str:
    ...
    rows = [
        f"◎片　　名　{movie.title}", f"◎年　　代　{movie.year or ''}",
        f"◎产　　地　{' / '.join(movie.countries)}", f"◎类　　别　{' / '.join(movie.genres)}",
...
def generate_description(config: dict[str, Any]) -> str:
    ...
    if douban_id:
        try:
            key = os.environ.get(str(config.get("description_api_key_env", "PT_GEN_API_KEY")), "")
            return ptgen_description(douban_id, str(config.get("description_api_base") or DESCRIPTION_API_DEFAULT), key)
        except RuntimeError as exc:
...
def torrent_trackers(config: dict[str, Any]) -> tuple[list[str], dict[str, Any]]:
    ...
    for tracker in trackers:
        ...
        if parsed.scheme.casefold() not in {"http", "https", "udp"} or not parsed.netloc:
            raise ValueError("Every torrent tracker must be an absolute HTTP(S) or UDP URL")
    return trackers, settings


class TorrentProgress:
    def __init__(self, status: Status, status_label: str,
                 detail: str, step: int = 7) -> None:
        self.status = status
    ...
    def __call__(self, *args: Any) -> None:
        ...
        update_status(
            self.status, self.status_label, self.detail,
            completed, total, self.step,
        )
...
def create_private_torrent(config: dict[str, Any], bt_dir: Path, output: Path,
                           status: Status | None = None,
                           status_label: str = "Creating torrent") -> str:
    ...
    try:
        ...
        loaded_trackers = {
            str(tracker) for tier in loaded.trackers for tracker in tier
        }
        if not set(trackers).issubset(loaded_trackers):
            raise RuntimeError("Generated torrent does not contain every configured tracker")
        if loaded.infohash != torrent.infohash:
            raise RuntimeError("Torrent info hash changed after writing and reading the file")
        if bool(settings.get("verify", True)):
            ...
            try:
                if not loaded.verify(bt_dir, callback=verify_progress, interval=1):
                    raise RuntimeError(f"Torrent piece verification failed for {bt_dir}")
            finally:
...
def pending_upload_count(config: dict[str, Any], screenshot_root: Path) -> int:
    ...
    for section in ("Comparison", "More"):
        files = bbcode.image_files(screenshot_root / section, missing_ok=True)
        ...
        section_failures = section_failures if isinstance(section_failures, dict) else {}
        if retry_only:
...
def build_variant(
    root: Path, variant: str, upload: bool, shared_description: str | None,
    ...
    config_file, config, document, variant_config = (
        context or load_variant_config(root, variant)
    )
    ...
    with status_context as status:
        ...
        uploaded_cache = bbcode.cached_screenshot_urls(config, screenshot_root)
        ...
        if upload:
            ...
            remote_root = bbcode.remote_release_folder(movie_file)
            upload_total = pending_upload_count(variant_config, screenshot_root)
            upload_done = 0

            def upload_event(event: str, path: Path, attempt: int,
                ...
                if event in {"success", "failed"}:
                    upload_done += 1
                update_status(
                    status, status_label, detail,
                    upload_done, upload_total, current_step,
                )
                if event == "success":
                    LOGGER.info("Screenshot uploaded: %s", path)
                elif event == "failed":
                    LOGGER.error("Screenshot skipped after retries: %s: %s", path, error)

            upload_args = (
                variant_config, config_file, screenshot_root, remote_root, token
            )
            if config_lock is None:
                uploaded_cache, failures = bbcode.upload_screenshots_cached(
                    *upload_args, document=document,
                    event_callback=upload_event, verbose=False,
                )
            else:
        ...
        md5_path.write_text(f"{movie_file.name} {md5_file(movie_file)}\n", encoding="ascii")
        LOGGER.info("MD5: %s", md5_path)
        next_stage("creating torrent")

        torrent_output = resolved_path(
            config.get("torrent_output") or bt_dir.parent / f"{movie_file.stem}.torrent",
            config_base,
        )
        info_hash = create_private_torrent(
            config, bt_dir, torrent_output,
    ...
    if failures:
        ...
        LOGGER.error("Failed screenshot uploads for %s: %s", variant, ", ".join(failed_names))
        CONSOLE.print(
...
def load_build_contexts(root: Path, selected: list[str]) -> dict[
    str, tuple[Path, dict[str, Any], dict[str, Any], dict[str, Any]]
]:
    path = pipeline_config_path(root)
    if not path.is_file():
...
def build(root: Path, selected: list[str], upload: bool) -> None:
    ...
    with progress:
        ...
        with ThreadPoolExecutor(
            ...
            futures = {
                executor.submit(
                    build_variant, root, variant, upload, description,
                    contexts[variant], reporters[variant], config_lock,
                ): variant
                for variant in selected
            }
...
def main() -> int:
    ...
    prepare_parser = subparsers.add_parser("prepare", help="create release directory structures/configs")
    ...
    prepare_parser.add_argument("--variant", choices=("all", "x264", "x265"), default="all")
    prepare_parser.add_argument("--force", action="store_true", help="overwrite existing generated configs")

    build_parser = subparsers.add_parser("build", help="generate final release artifacts")
    build_parser.add_argument("root", type=Path, help="Blu-ray source directory")
    build_parser.add_argument("--variant", choices=("all", "x264", "x265"), default="all")
    build_parser.add_argument("--upload", action="store_true", help="upload screenshots to TTG")
    ...
    try:
        if args.command == "prepare":
            prepare(
                args.root, args.imdb_id, args.douban_id, variants(args.variant),
                args.force, args.title,
            )
        else:
