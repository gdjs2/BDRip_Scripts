...
if __package__:
    ...
    from .crf_plot import PALETTE, make_figure
else:
    from crf_model import CRF_VALUES, fit_models, predict
    from crf_plot import PALETTE, make_figure
...
class InteractivePlot(QWidget):
    def __init__(self):
        ...
        self.unpin_action.setToolTip("Release the selected CRF and resume mouse estimates")
    ...
    def set_report(self, report: dict) -> None:
        ...
        self.models = {codec: analysis.get("models") or fit_models(analysis["rows"])
        ...
        self.cursor = bitrate_axes.axvline(13, color="#475569", linewidth=1, linestyle=":",
                                           visible=False, animated=True)
        self.tooltip = bitrate_axes.text(0.02, 0.98, "", transform=bitrate_axes.transAxes,
                                        va="top", fontsize=10, visible=False, animated=True,
                                        bbox={"boxstyle": "round,pad=0.5", "fc": "white", "ec": "#94a3b8", "alpha": 0.95})
        self.tooltip.set_in_layout(False)
        self.markers = {}
        for codec in self.models:
            color, shape = PALETTE[codec]
            for axes, metric in ((qp_axes, "average_qp"), (bitrate_axes, "average_bitrate_mbps")):
                marker, = axes.plot([], [], marker=shape, color=color, linestyle="none", markersize=7,
    ...
    def event_crf(self, event) -> float | None:
        crf = event.xdata
        if (event.inaxes not in self.axes or crf is None or not math.isfinite(crf)
                or not 0 <= crf <= 51 or self.toolbar.mode):
            return None
        return crf
    ...
    def show_estimate(self, crf: float) -> None:
        self.hover_crf = crf
        self.estimates = {codec: predict(model, crf) for codec, model in self.models.items()}
