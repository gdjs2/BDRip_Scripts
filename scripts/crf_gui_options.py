...
class EncoderOptions(QWidget):
    def __init__(self, codec: str, settings: dict):
        ...
        layout.addWidget(QLabel(f"{codec} parameters — key=value entries separated by colons or newlines"))
        self.params = QPlainTextEdit()
        self.params.setFont(QFontDatabase.systemFont(QFontDatabase.SystemFont.FixedFont))
        ...
        self.options.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
    ...
    def load(self, settings: dict) -> None:
        ...
        self.params.setPlainText(":".join(f"{key}={value}" for key, value in settings["params"].items()))
    ...
    def values(self) -> dict:
        ...
        for row in range(self.options.rowCount()):
            key, value = [self.options.item(row, col).text().strip()
                          if self.options.item(row, col) else "" for col in (0, 1)]
            if not key and not value:
                continue
            if not key or not value:
                raise ValueError(f"{self.codec}: each additional option needs a name and a value")
            if key in options:
        ...
        return {
            ...
            "params": ":".join(line.strip().strip(":") for line in self.params.toPlainText().splitlines() if line.strip()),
            "options": options,
        }
...
class EncoderSettingsDialog(QDialog):
    def __init__(self, config: dict, parent=None):
        ...
        layout = QVBoxLayout(self)
        label = QLabel("Changes apply to videos added after you save. Each queued task keeps its captured settings.")
