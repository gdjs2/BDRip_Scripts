...
_X264_SUMMARY = re.compile(
    r"frame\s+([IPB]):\s*(\d+)\s+Avg QP:\s*(-?\d+(?:\.\d+)?)"
)
...
def check_encoder(codec: str, settings: dict[str, Any]) -> dict[str, Any]:
    ...
    pixel_format = settings.get("pixel_format", "yuv420p" if codec == "x264" else "yuv420p10le")
    if pixel_format not in formats:
        raise EncodingError(
            f"{library} does not support pixel format {pixel_format!r}; "
            f"available formats: {', '.join(formats)}"
        )
    return {"library": library, "pixel_format": pixel_format, "pyav_version": av.__version__}
...
def _crop_dimensions(crop: str | None, width: int, height: int) -> tuple[int, int]:
    ...
    if out_width <= 0 or out_height <= 0 or x + out_width > width or y + out_height > height:
        raise EncodingError(f"Crop {crop!r} is outside the {width}x{height} source")
    return out_width, out_height


def validate_source_settings(
    source: dict[str, Any], codec: str, settings: dict[str, Any], crop: str | None = None
) -> tuple[int, int]:
...
def _encoder_options(job: dict[str, Any], csv_name: str) -> dict[str, str]:
    ...
    if forbidden:
        raise EncodingError(
            "Custom parameters cannot override the CRF experiment: " + ", ".join(sorted(forbidden))
        )
    options = {str(key): str(value) for key, value in settings.get("options", {}).items()}
    controlled = {"crf", "preset", "tune", "profile", "level", "x264-params", "x265-params"}
    if set(options) & controlled:
        raise EncodingError("Use the named settings fields for CRF, preset, profile, level and encoder params")
    ...
    options.update({"crf": str(job["crf"]), "preset": str(settings.get("preset", default_preset))})
    ...
    if any(":" in key or "=" in key or ":" in value for key, value in params.items()):
        raise EncodingError("Encoder parameter keys and values cannot contain ':'; use ',' for paired values")
    if params:
        options[f"{codec}-params"] = ":".join(f"{key}={value}" for key, value in params.items())
    return options


def parse_x264_summary(logs: list[tuple[int, str, str]]) -> tuple[dict[str, float], dict[str, int]]:
    qp: dict[str, float] = {}
    counts: dict[str, int] = {}
    ...
    with path.open(encoding="utf-8-sig", newline="") as handle:
        ...
        for row in reader:
            ...
            try:
                ...
                raise EncodingError("x265 frame statistics contain an invalid QP value") from exc
            if not math.isfinite(value):
                raise EncodingError("x265 frame statistics contain a non-finite QP value")
...
def _reject_invalid_options(logs: list[tuple[int, str, str]]) -> None:
    invalid = [message.strip() for _level, _name, message in logs if _INVALID_OPTION.search(message)]
    if invalid:
        raise EncodingError("Encoder rejected parameters: " + "; ".join(invalid))


def _encode_frames(
    job: dict[str, Any], source: dict[str, Any], options: dict[str, str],
    ...
    try:
        with av.open(str(job["input"]), mode="r") as container:
            ...
            if not stream.time_base:
                raise EncodingError("The selected video stream has no timestamp time base")
            if target_start > stream_start:
                container.seek(int(target_start / stream.time_base), stream=stream, backward=True)

            width, height = validate_source_settings(source, job["codec"], job["settings"], job.get("crop"))
            ...
            encoder.open()
            if encoder.options:
                raise EncodingError("Unrecognized encoder options: " + ", ".join(sorted(encoder.options)))
            ...
            def submit(frame: Any, timestamp: Fraction, duration: Fraction) -> None:
                ...
                nonlocal graph, crop_source, crop_sink
                if first_time is None:
                    first_time = timestamp
                if duration <= 0:
                    raise EncodingError("Decoded video contains a frame with no positive presentation duration")
                if job.get("crop"):
                    if graph is None:
                        graph = av.filter.Graph()
                        crop_source = graph.add_buffer(template=frame)
                        crop_filter = graph.add("crop", f"{job['crop']}:exact=1")
                        crop_sink = graph.add("buffersink")
                        crop_source.link_to(crop_filter)
                        crop_filter.link_to(crop_sink)
                        graph.configure()
                ...
                if ticks.denominator != 1:
                    raise EncodingError("Decoded frame timestamps cannot be represented exactly in the stream time base")
                frame.pts = int(ticks)
            ...
            for frame in decoder:
                ...
                if frame.pts is None or not frame.time_base:
                    raise EncodingError("Decoded video frames have no presentation timestamps")
                timestamp = frame.pts * Fraction(frame.time_base)
                if previous_time is not None and timestamp <= previous_time:
                    raise EncodingError("Decoded video presentation timestamps are not strictly increasing")
...
def encode_sample(job: dict[str, Any]) -> dict[str, Any]:
    ...
    if job["start"] < 0 or job["duration"] <= 0:
        raise EncodingError("Sample start must be nonnegative and duration must be positive")
    if not 0 <= job["crf"] <= 51:
        ...
        with tempfile.TemporaryDirectory(prefix="crf-encode-") as temporary:
            with contextlib.chdir(temporary):
                ...
                if sum(counts.values()) != result["frames"]:
                    raise EncodingError("Encoder QP statistics do not account for every encoded video frame")
