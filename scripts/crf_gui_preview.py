...
from PySide6.QtWidgets import QDialog, QHBoxLayout, QLabel, QPushButton, QScrollArea, QStackedWidget, QVBoxLayout, QWidget

if __package__:
    from .crf_model import METHOD
    from .crf_progress import read_json
else:
    from crf_model import METHOD
    from crf_progress import read_json
...
class ImagePreview(QWidget):
    def __init__(self):
        ...
        toolbar = QHBoxLayout()
        self.buttons = []
        for label, callback in (("Fit", lambda: self.set_zoom(None)), ("100%", lambda: self.set_zoom(1.0)),
    ...
    def _render(self) -> None:
        ...
        if self.original.isNull():
            self.canvas.clear()
            self.canvas.setText("Measured points appear as encoding finishes. Curves appear after CRF 13 and 20 are complete.")
            self.canvas.resize(available)
            self.zoom_label.clear()
            return
        fit = min(available.width() / self.original.width(), available.height() / self.original.height())
        self.scale = max(0.01, min(1.0, fit)) if self.zoom is None else self.zoom
        size = self.original.size() * self.scale
        self.canvas.setPixmap(self.original.scaled(size, Qt.AspectRatioMode.KeepAspectRatio,
                                                  Qt.TransformationMode.SmoothTransformation))
        self.canvas.resize(size)
        self.zoom_label.setText(f"{'Fit · ' if self.zoom is None else ''}{self.scale:.0%}")
    ...
    def set_report(self, report: dict) -> None:
        if report.get("method") == METHOD:
            if self.chart is None:
                ...
                if __package__:
                    from .crf_gui_plot import InteractivePlot
                else:
                    from crf_gui_plot import InteractivePlot
