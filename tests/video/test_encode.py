...
from bdrip.video import encode as backend
        ...
        for field in ("params", "options"):
            ...
            with self.subTest(field=field), contextlib.redirect_stderr(io.StringIO()):
                with self.assertRaisesRegex(
                    backend.EncodingError, "[Uu]nrecognized|rejected"
                ):
    ...
    def assert_exact_crop(self, codec, *, top, left=0, width=1920):
                ...
                if frame is not None:
                    ...
                    def pixel(x, y):
                        ...
                        return int.from_bytes(
                            data[offset : offset + sample_bytes], "little"
                        )
        ...
        class RecordingAv:
            CodecContext = SimpleNamespace(
                create=lambda name, mode: RecordingEncoder(
        ...
        job = {
            "input": str(self.sources[top]),
            "start": 0.0,
            "duration": 0.04,
            "codec": codec,
            "crf": 18,
            "crop": f"{width}:804:{left}:{top}",
            "settings": settings,
        }
        with (
            patch.object(backend, "av", RecordingAv()),
        ...
        self.assertEqual(
            [(frame.width, frame.height) for frame in decoded], [(width, 804)]
        )
