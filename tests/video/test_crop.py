...
from bdrip.video.crop import _LumaBounds, detect_crop
from bdrip.video.encode import EncodingError, inspect_video
...
class CropDetectionTests(unittest.TestCase):
    ...
    def detect(self, samples=None, *, seconds=2.0):
        ...
        samples = samples or [
            {"start": 0.0, "duration": source["duration"], "kind": "representative"}
        ]
        return detect_crop(
            self.path, 0, samples, source, {"limit": 24 / 255, "seconds": seconds}
        )
    ...
    def test_variable_aspect_ratios_use_union_of_representative_and_stress_windows(
        self,
    ):
    ...
    def test_black_and_tiny_dark_scene_content_keep_full_frame(self):
        for rectangle, reason in (
            (None, "no_content"),
            ((60, 40, 67, 55), "small_content"),
        ):
            with self.subTest(rectangle=rectangle):
                make_video(self.path, [rectangle] * 3)
                result = self.detect()
