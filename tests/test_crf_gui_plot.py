...
try:
    ...
    from scripts.crf_gui_preview import FigureDialog, FigurePreview
except ImportError:
    QApplication = None

from scripts.crf_model import fit_models
from scripts.crf_progress import write_json
from tests.test_crf_model import anchors
...
def report(x264=None, x265=None):
    ...
    return {"method": "two_point", "state": "complete", "source": {"path": "Movie.mkv"}, "codecs": codecs}
...
class InteractivePlotTests(unittest.TestCase):
    ...
    def position(self, crf, height=0.5):
        ...
        axes = self.chart.axes[1]  # Matplotlib dispatches twin-axis events to the upper axes.
    ...
    def click(self, crf, button=None):
        QTest.mouseClick(self.chart.canvas, button or Qt.MouseButton.LeftButton, pos=self.position(crf))
        self.app.processEvents()

    def test_hover_uses_continuous_crf_and_both_models_at_any_mouse_height(self):
        with mock.patch("subprocess.Popen", side_effect=AssertionError("Hover must not run encoders")):
            for height in (0.2, 0.8):
                self.hover(16.37, height)
                crf = self.chart.hover_crf
                self.assertAlmostEqual(crf, 16.37, delta=0.02)
                first, second = (self.chart.estimates[codec] for codec in ("x264", "x265"))
                self.assertAlmostEqual(first["average_qp"], 5 + crf)
                self.assertAlmostEqual(second["average_qp"], 9 + crf)
                self.assertAlmostEqual(first["average_bitrate_mbps"], 16 * 0.25 ** ((crf - 13) / 7))
                self.assertAlmostEqual(second["average_bitrate_mbps"], first["average_bitrate_mbps"] / 2)
    ...
    def test_full_draw_does_not_request_a_recursive_qt_repaint(self):
        self.hover(16.5)
        with mock.patch.object(self.chart.canvas, "blit", wraps=self.chart.canvas.blit) as repaint:
    ...
    def test_click_pins_estimates_until_moved_or_released(self):
        with mock.patch("subprocess.Popen", side_effect=AssertionError("Pinning must not run encoders")):
            self.click(16.37)
            ...
            self.assertAlmostEqual(estimates["x264"]["average_bitrate_mbps"], 16 * 0.25 ** ((pinned - 13) / 7))
    ...
    def test_pin_survives_resize_zoom_and_new_models_but_clears_on_task_switch(self):
        ...
        self.assertAlmostEqual(self.chart.estimates["x264"]["average_bitrate_mbps"],
                               16 * 0.25 ** ((pinned - 13) / 7))
        self.preview.set_report(report(x265=anchors(qps=(22, 29), rates=(8, 2))))
        self.app.processEvents()
        self.assertEqual(self.chart.pinned_crf, pinned)
    ...
    def test_partial_and_missing_b_qp_do_not_create_estimated_qp(self):
        for rows in (anchors()[:1], anchors(qps=(18, None))):
            ...
            if len(rows) == 2:
                self.assertIsNotNone(self.chart.estimates["x264"]["average_bitrate_mbps"])
                self.assertTrue(self.chart.markers["x264", "average_bitrate_mbps"].get_visible())
    ...
    def test_resize_zoom_and_live_update_keep_hover_coordinates_correct(self):
        ...
        self.assertEqual(self.chart.axes[0].get_xlim(), (15, 18))
        self.hover(16.6)
        self.assertAlmostEqual(self.chart.estimates["x264"]["average_qp"], 21.6, delta=0.02)
    ...
    def test_larger_view_loads_and_refreshes_models_without_a_saved_png(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIs(dialog.preview.stack.currentWidget(), dialog.preview.chart)
