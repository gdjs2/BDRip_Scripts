...
from bdrip.crf.model import fit_models, predict
...
class PlotTests(unittest.TestCase):
    ...
    def test_qp_and_bitrate_share_one_plot_with_two_scales_and_distinct_line_styles(
        self,
    ):
        ...
        for axes, metric in ((qp, "QP"), (rate, "bitrate")):
            ...
            self.assertEqual(
                ...
                [f"x264 {metric}", f"x265 {metric}"],
            )
            self.assertEqual(
                [points.get_label() for points in axes.collections],
                [f"x264 {metric} measured", f"x265 {metric} measured"],
            )
