...
try:
    ...
    from scripts.crf_gui import EncoderSettingsDialog, Window
    from scripts.crf_queue import TaskQueue
except ImportError:
    QApplication = None

from scripts.crf_config import load_config, validate_config
from scripts.crf_model import predict
from tests.test_crf_search import make_media
...
class QueueTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = validate_config({
            "video": {"crop": None},
            "codecs": {
                "x264": {"preset": "ultrafast", "params": {"threads": 1, "bframes": 2, "b-adapt": 0, "rc-lookahead": 4}},
    ...
    def wait_until(self, predicate, timeout=45):
        ...
        while not predicate() and time.monotonic() < deadline:
            QTest.qWait(20)
        self.assertTrue(predicate(), "Timed out: " + str([(t.state, t.error) for t in self.queue.tasks]))

    def test_saved_queue_snapshots_configs_and_keeps_artifacts_on_remove(self):
        first, second = self.queue.add([str(self.source), str(self.source)], self.config, "both")
        ...
        try:
            self.assertEqual([task.id for task in restored.tasks], [first.id, second.id])
            self.assertEqual(restored.tasks[0].config["video"]["cropdetect"]["seconds"], 3)
    ...
    def test_encoder_edits_are_saved_and_used_by_background_encoders(self):
        ...
        dialog.accept()
        self.assertEqual(dialog.result(), dialog.DialogCode.Accepted, dialog.error.text())
        # Crop edits made before opening the encoder dialog are preserved.
        self.window.crop.setText("94:62:1:1")
        with (mock.patch("scripts.crf_gui.EncoderSettingsDialog", return_value=dialog),
        ...
        with mock.patch("scripts.crf_gui.QFileDialog.getOpenFileNames", return_value=([str(self.source)], "")):
        ...
        with mock.patch("scripts.crf_gui.QFileDialog.getSaveFileName", return_value=(str(saved), "")):
        ...
        for codec, analysis in report["codecs"].items():
            for row in analysis["rows"]:
                ...
                self.assertEqual(options["preset"], "superfast" if codec == "x264" else "veryfast")
                self.assertIn("bframes=2", options[f"{codec}-params"])
                if codec == "x264":
                    self.assertEqual((options["level"], options["tune"], options["threads"]), ("4.0", "grain", "1"))
    ...
    def test_larger_preview_fits_zooms_and_refreshes_without_blocking_window(self):
        ...
        self.assertGreater(preview.preview.image.scroll.horizontalScrollBar().maximum(), 0)
        ...
        self.assertEqual(preview.preview.image.original.toImage().pixelColor(0, 0).name(), "#123456")
    ...
    def test_failure_does_not_block_later_videos_and_gui_remains_responsive(self):
        ...
        try:
            ...
            timer.stop()
        self.assertEqual([task.state for task in self.queue.tasks], ["failed", "completed", "completed"])
        ...
        for task, count in ((first, 4), (second, 2)):
            ...
            for filename in ("config.json", "progress.json", "sample-progress.json", "results.json",
                             "summary.csv", "estimates.csv", "qp-bitrate.png", "qp-bitrate.svg", "task.log"):
            ...
            self.assertEqual(json.loads((output / "results.json").read_text())["qp_frame_type"], "B")
        self.window.tree.setCurrentItem(self.window.items[first.id])
        self.window.refresh_details()
        self.assertFalse(self.window.figure.image.original.isNull())
        self.assertIs(self.window.figure.stack.currentWidget(), self.window.figure.chart)
        ...
        for index, codec in enumerate(("x264", "x265")):
            ...
            self.assertAlmostEqual(float(self.window.models.table.item(index, 1).text()),
                                   estimated["average_qp"], delta=0.0005)
            self.assertAlmostEqual(float(self.window.models.table.item(index, 2).text()),
                                   estimated["average_bitrate_mbps"], delta=0.0005)
    ...
    def test_cancellation_reaps_encoder_preserves_results_and_retry_reuses_cache(self):
        task = self.queue.add([str(self.source)], self.config, "x264")[0]
        self.queue.start()
        self.wait_until(lambda: task.progress.get("crf") == 20 and task.progress.get("stage") == "encoding"
        ...
        saved_logs = {path.name: path.read_bytes() for path in (output / "logs").glob("*.log")}
    ...
    def test_closing_window_cancels_active_task_and_preserves_waiting_queue(self):
        task, waiting = self.queue.add([str(self.source), str(self.source)], self.config, "x264")
        self.queue.start()
        self.wait_until(lambda: task.progress.get("stage") == "encoding" and task.progress.get("encoder_pid"))
        self.window.close()
        self.assertTrue(self.window.closing)
        self.wait_until(lambda: self.queue.active is None and not self.window.isVisible())
        self.assertEqual(task.state, "cancelled")
        self.assertEqual(waiting.state, "queued")
        restored = TaskQueue(self.queue.workspace)
        try:
            self.assertEqual([item.state for item in restored.tasks], ["cancelled", "queued"])
            self.assertFalse(restored.enabled)
        finally:
            restored.close()
    ...
    def test_recovery_and_corrupt_queue_do_not_discard_saved_tasks(self):
        ...
        restored.close()
        (self.queue.output(task) / "progress.json").write_text(json.dumps({
            "state": "complete", "message": "Completed", "elapsed_seconds": 42.5,
        }))
        restored = TaskQueue(self.queue.workspace)
    ...
    def test_old_queue_migrates_waiting_tasks_and_retains_historical_results_on_retry(self):
        waiting, history = self.queue.add([str(self.source), str(self.source)], self.config, "x264")
        ...
        original = {"results.json": '{"schema_version":3}', "task.log": "old sweep log",
