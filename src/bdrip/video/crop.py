...
from .probe import EncodingError, _stream_start, _video_stream
...
class ExactCrop:
    """Reuse one PyAV filter graph without rounding crop dimensions or offsets."""

    def __init__(self, crop: str | None):
        self.crop = crop
        self.graph = self.source = self.sink = None
    ...
    def apply(self, frame: Any) -> Any:
        ...
        if self.graph is None:
            ...
            self.source = self.graph.add_buffer(template=frame)
            crop_filter = self.graph.add("crop", f"{self.crop}:exact=1")
            self.sink = self.graph.add("buffersink")
            self.source.link_to(crop_filter)
            crop_filter.link_to(self.sink)
            self.graph.configure()
        self.source.push(frame)
        result = self.sink.pull()
        width, height, _, _ = map(int, self.crop.split(":"))
        if (result.width, result.height) != (width, height):
            raise EncodingError(
                f"Crop filter produced {result.width}x{result.height}; "
...
class _LumaBounds:
    ...
    def read(self, frame: Any) -> Bounds | None:
        ...
        if any(key.startswith("lavfi.bbox.") for key in frame.metadata):
            raise ValueError(
                "Input frames already carry ambiguous bounding-box metadata"
            )
        self.input.push(frame)
        measured = self.output.pull()
        if measured.format.components[0].bits != depth:
            raise ValueError(
                "The bounding-box filter cannot preserve source luminance bit depth"
            )
        ...
        try:
            ...
            raise ValueError(
                "The bounding-box filter returned incomplete pixel bounds"
            ) from exc
...
def _scan_window(
    ...
    diagnostics = dict(
        window, frames=0, content_frames=0, black_frames=0, uncertain_frames=0
    )
    ...
    try:
        for frame in decoder:
            ...
            if (frame.width, frame.height) != (source["width"], source["height"]):
                raise EncodingError(
                    "The input changes video dimensions during crop detection"
                )
            if frame.pts is None or frame.time_base is None:
                raise EncodingError(
                    "Crop detection requires video presentation timestamps"
                )
            timestamp = frame.pts * Fraction(frame.time_base)
            if previous_time is not None and timestamp <= previous_time:
                raise EncodingError(
                    "Video presentation timestamps are not increasing during crop detection"
                )
    ...
    diagnostics.update(
        {
            ...
            "pixel_format": reader.format,
            "bit_depth": reader.depth,
...
def detect_crop(
    ...
    seconds = _number(
        settings.get("seconds", 2.0), "Crop detection seconds", positive=True
    )
    width, height = source["width"], source["height"]
    windows = _windows(samples, source["duration"], seconds)
    result: dict[str, Any] = {
        "mode": "auto",
        "detector": "pyav-bbox-luma",
        "crop": None,
        "width": width,
        "height": height,
        "limit": limit,
    ...
    if "bbox" not in av.filter.filters_available:
        return finish(
            ...
            "This PyAV build has no bbox filter; kept the full frame",
        )
    bounds: Bounds | None = None
    uncertainty: set[str] = set()
    try:
        ...
        with av.logging.Capture(local=False):
            with av.open(str(Path(path).expanduser().resolve()), mode="r") as container:
                ...
                for window in windows:
                    if check_cancelled:
                        check_cancelled()
                    details, content, problems = _scan_window(
                        container, stream, source, window, limit, check_cancelled
                    )
    ...
    if uncertainty:
        ...
        return finish(
            "uncertain_detection",
            "Some sampled frames could not be measured reliably; kept the full frame",
        )
    if bounds is None:
        return finish(
            "no_content",
            "All sampled frames were below the luminance threshold; kept the full frame",
        )
    x1, y1, x2, y2 = bounds
    if bounds == (0, 0, width - 1, height - 1):
        return finish(
            "full_frame_content",
            "Observed content reaches the full frame; no consistent black margins",
        )
    active_width, active_height = x2 - x1 + 1, y2 - y1 + 1
    if (
        active_width * 2 < width
        or active_height * 2 < height
        or active_width * active_height * 2 < width * height
    ):
        return finish(
            "small_content",
            "Detected content is too small to distinguish margins from a dark scene; kept the full frame",
        )
    ...
    result.update(
        crop=f"{active_width}:{active_height}:{x1}:{y1}",
        width=active_width,
        height=active_height,
    )
    return finish(
        "detected_margins",
        "Exact crop preserves the union of content in all detection windows",
    )
