...
def make_figure(report: dict, figure: Figure | None = None) -> Figure:
    ...
    legend_handles = []
    grid = [
        CRF_VALUES[0] + (CRF_VALUES[1] - CRF_VALUES[0]) * i / 200 for i in range(201)
    ]
    ...
    for codec, analysis in report["codecs"].items():
        ...
        for axes, metric, model, name, style in (
            (qp_axes, "average_qp", "qp", "QP", "-"),
            ...
            if models[model]:
                (line,) = axes.plot(
                    grid,
                    [row[metric] for row in estimates],
                    color=color,
                    linewidth=1.8,
                    linestyle=style,
                    label=f"{codec} {name}",
            ...
            if measured:
                points = axes.scatter(
                    [row["crf"] for row in measured],
                    [row[metric] for row in measured],
                    edgecolors=color,
                    facecolors=color if model == "qp" else "none",
                    marker=marker,
                    s=40,
                    zorder=3,
                    label=f"{codec} {name} measured",
                )
                if not models[model]:
    ...
    if not has_qp:
        message = (
            "B-frame QP unavailable" if missing_b_frames else "Waiting for measurements"
        )
    ...
    if not has_bitrate:
        if has_qp:
            bitrate_axes.text(
                0.98,
                0.04,
                "Waiting for bitrate measurements",
                ha="right",
                va="bottom",
