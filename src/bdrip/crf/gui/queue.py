...
from bdrip.crf.config import validate_config
...
class Task:
    ...
    def folder(self) -> str:
        stem = (
            re.sub(r"[^\w.-]+", "_", Path(self.video).stem)[:60].strip(".") or "video"
        )
    ...
    def percent(self) -> int:
        ...
        if not total:
            return 0
        fraction = (
            self.progress.get("sample_fraction", 0) if self.state == "running" else 0
        )
        return min(
            99,
            max(0, int(100 * (self.progress.get("completed", 0) + fraction) / total)),
        )
...
class TaskQueue(QObject):
    ...
    def __init__(self, workspace: Path, parent=None):
        ...
        if not self.lock.tryLock(0):
            raise ValueError(
                "This queue is already open in another window. Choose a different --workspace."
            )
        ...
        try:
            ...
            if not worker_lock.tryLock(0):
                raise ValueError(
                    "A background task in this queue is still running. Reopen the queue after it finishes."
                )
            worker_lock.unlock()
            self._load()
    ...
    def _load(self) -> None:
        ...
        try:
            ...
            for item in data["tasks"]:
                ...
                if (
                    not re.fullmatch(r"[0-9a-f]{32}", task.id)
                    or task.state not in STATES
                    or task.codec not in {"both", "x264", "x265"}
                ...
                if task.state in {"running", "cancelling"}:
                    ...
                    task.error = (
                        ...
                        else task.progress.get(
                            "message", "Previous run was interrupted"
                        )
                    )
                    task.finished = (task.started or time.time()) + max(
                        0, task.progress.get("elapsed_seconds", 0)
                    )
            ...
            raise ValueError(
                f"Cannot read {path}: {exc}. The existing queue has been left intact."
            ) from exc
    ...
    def _advance(self) -> None:
        ...
        try:
            ...
            process.setStandardOutputFile(
                str(output / "task.log"), QIODevice.OpenModeFlag.Append
            )
            ...
            for key, value in {
                ...
                "TERM": "dumb",
                "MPLBACKEND": "Agg",
                "MPLCONFIGDIR": str(self.workspace / ".matplotlib"),
            }.items():
    ...
    def _process_error(self, process: QProcess, error) -> None:
        if process is self.process and error == QProcess.ProcessError.FailedToStart:
            self._finish_task(
                "failed", f"Cannot start Python worker: {process.errorString()}"
            )
    ...
    def _finished(self, process: QProcess, code: int, status) -> None:
        ...
        if (
            ...
            state, error = (
                "cancelled",
                "Cancelled; completed measurements are available for retry",
            )
        else:
            state = "failed"
            error = (
                self.active.progress.get("message", "Task failed")
    ...
    def close(self) -> None:
        if self.active:
            raise RuntimeError(
                "Cancel the active task and wait for its finished signal before closing"
            )
        self.timer.stop()
        self.lock.unlock()
