...
from .artwork import BANNER_B64, BOTTOM_B64
from .media import aspect_ratio_text, compact_audio_codec
...
def get_audio_text(track, channel: float, max_length: int = 48) -> str:
    language = (
        track.other_language[0] if track.other_language else track.language or "Unknown"
    )
...
def get_file_metadata(file_path: Path) -> dict:
    ...
    for track in media_info.tracks:
        if track.track_type == "General":
            ...
            aspect = aspect_ratio_text(
                ...
                or (
                    track.other_display_aspect_ratio[0]
                    if track.other_display_aspect_ratio
                    else None
                )
            )
            metadata["resolution"] = f"{track.width}x{track.height} ({aspect})"
    ...
    if "subtitles" in metadata:
        ...
        metadata["subtitles"] = " | ".join(
            [f"{codec} ({metadata['subtitles'][codec]})" for codec in codec_list]
        )  # type: ignore
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--encoder", default="WiKi", help="encoder name (default: WiKi)"
    )
