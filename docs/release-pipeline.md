# Release documents and pipeline

`bdrip bbcode` builds a UTF-8 WiKi-style BBCode release post. It
fetches title, plot,
...
Run these commands from the repository after `uv sync`.
...
```powershell
uv run bdrip bbcode release.json -o post.bbcode.txt
```

Upload every screenshot and embed the URLs returned by TTG:

```powershell
uv run bdrip bbcode release.json --upload
```
...
```powershell
uv run bdrip release prepare "D:\Movie.Source.Directory" `
...
```powershell
uv run bdrip release build "D:\Movie.Source.Directory"
```
